...
container_name = os.getenv('AZURE_CONTAINER_NAME_TEXT', 'landing/kafka/text')  # Nome do container onde deseja gravar os blobs
...
while True:
    for message in consumer.consume(timeout=1):
        ...
        blob_name = f"{message.topic()}_{message.offset()}.txt"
